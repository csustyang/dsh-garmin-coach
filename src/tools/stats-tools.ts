 ...
 */
export function defineStatsTools(
  ctx: StatsToolContext,
): ReturnType<typeof defineGarminTool>[] {
  ...
  return [
    ...
    defineGarminTool({
      ...
      description:
        '查看最近 N 次活动（可按运动类型筛选）。返回活动列表含距离、配速、心率、时间。',
      parameters: {
    ...
    defineGarminTool({
      ...
      description:
        '查询指定距离的最好成绩（配速最快）。例如"十公里最好成绩"传 distanceMeters=10000。',
      parameters: {
    ...
    defineGarminTool({
      ...
      description:
        '保存 AI 生成的训练计划到缓存（data/training-plan.json）。' +
    ...
    defineGarminTool({
      ...
      description:
        '打卡/取消打卡一个训练任务。传入 taskId（如 w1-d1），切换该任务的完成状态。' +
    ...
    defineGarminTool({
      ...
      description:
        '从历史版本恢复训练计划。传入 garmin_plan_history 返回的 file 文件名，' +
    ...
    defineGarminTool({
      ...
      description:
        '记录一条训练日记。用户完成一次训练后可记录感受/评分，可关联某个训练任务。' +
    ...
    defineGarminTool({
      name: 'garmin_diary',
      description:
        '查看训练日记。可按日期范围或最近 N 条。返回日记条目（含日期、感受、评分、关联任务）。',
      parameters: {
