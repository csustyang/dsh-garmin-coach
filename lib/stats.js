...
async function sortedActivities(store) {
    ...
    return Object.values(data.activities).sort((a, b) => b.startTime.localeCompare(a.startTime));
}
/** 按运动类型过滤 */
...
export async function dashboardSummary(store) {
    const data = await store.read();
    const activities = Object.values(data.activities).sort((a, b) => b.startTime.localeCompare(a.startTime));
