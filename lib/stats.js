...
export function formatPace(secPerKm) {
    if (secPerKm === undefined || secPerKm <= 0)
        return '—';
...
function hydrateFromRaw(a) {
    ...
    return {
        ...
        gradeAdjustedPaceSecPerKm: a.gradeAdjustedPaceSecPerKm ?? toPace(raw.avgGradeAdjustedSpeed),
...
async function sortedActivities(store) {
...
/** 按运动类型过滤 */
function filterBySport(activities, sport, sports) {
    if (sport)
...
/** 按距离范围过滤（米）*/
function filterByDistance(activities, minMeters, maxMeters) {
    return activities.filter((a) => {
        if (minMeters !== undefined && (a.distanceMeters ?? 0) < minMeters)
            return false;
        if (maxMeters !== undefined && (a.distanceMeters ?? 0) > maxMeters)
            return false;
        return true;
    });
}
/** 最近 N 次活动 */
export async function recentActivities(store, args) {
...
/** 某距离的最佳配速成绩 */
export async function bestPace(store, args) {
    const activities = await sortedActivities(store);
...
/** 距离统计（总距离、次数、平均配速）*/
export async function distanceStats(store, args) {
    const activities = await sortedActivities(store);
...
export async function dailyStats(store, args) {
    const data = await store.read();
    let dailies = Object.values(data.daily).sort((a, b) => b.date.localeCompare(a.date));
    if (args.days) {
    ...
    const avg = (arr) => {
        const nums = arr.filter((x) => x !== undefined);
    ...
    return {
        ...
        avgStress: avg(dailies.filter((d) => d.stressAvg != null && d.stressAvg > 0).map((d) => d.stressAvg)),
        // HRV/readiness 字段已停用
...
/** 运动类型分布 */
export async function sportBreakdown(store, days) {
    const activities = await sortedActivities(store);
 ...
 */
export async function dashboardSummary(store) {
    const data = await store.read();
    ...
    const recent = activities.map((a) => {
        const { raw, ...rest } = a;
        const at = raw?.activityType;
        return { ...rest, parentTypeId: at?.parentTypeId };
    });
    const weekly = [
        {
            week: '本周',
            km: Math.round((activities.filter((a) => new Date(a.startTime).getTime() >= weekStart)
                .reduce((s, a) => s + (a.distanceMeters ?? 0), 0) / 1000) * 10) / 10,
            runs: activities.filter((a) => a.sport === 'running' &&
                new Date(a.startTime).getTime() >= weekStart).length,
        },
    ];
    // 各距离 PB
    const bestPaces = {
...
export async function paceByDistance(store) {
    const data = await store.read();
    ...
    return segments.map(function (seg) {
        ...
        const paces = matches.map(function (a) { return a.avgPaceSecPerKm; }).filter(function (p) { return p && p > 0; });
        if (paces.length === 0) {
...
/** 配速分布（按时间区间统计次数）*/
export async function paceDistribution(store) {
    const data = await store.read();
    ...
    return buckets.map(function (b) {
        ...
        const hrs = matches.map(function (a) { return a.avgHr; }).filter(function (h) { return h !== undefined; });
        return {
            range: b.range,
            count: matches.length,
...
export async function hrPaceRelationship(store) {
    const data = await store.read();
    const runs = Object.values(data.activities).filter(function (a) {
...
export async function trainingLoad(store) {
    const data = await store.read();
    ...
    items.forEach(function (i) {
        ...
        if (!byWeek[wk])
            byWeek[wk] = { load: 0, durationMin: 0 };
    ...
    acts.forEach(function (a) {
        ...
        const wk = monday.toISOString().slice(0, 10);
        if (byWeek[wk])
            byWeek[wk].durationMin += (a.durationSec ?? 0) / 60;
    });
...
/** 距离分布（按距离区间统计次数）*/
export async function distanceDistribution(store) {
    const data = await store.read();
...
/** 周环比（本周 vs 上周）*/
export async function weekOverWeek(store) {
    const data = await store.read();
    const now = new Date();
    const thisMon = new Date(now);
    thisMon.setDate(now.getDate() - now.getDay() + 1);
    const lastMon = new Date(thisMon);
    lastMon.setDate(thisMon.getDate() - 7);
    const preLastMon = new Date(lastMon);
    preLastMon.setDate(lastMon.getDate() - 7);
    const acts = Object.values(data.activities);
    ...
    function stats(arr) {
        if (arr.length === 0)
            return { km: 0, runs: 0, avgPace: null, avgHr: null, load: 0 };
        const km = arr.reduce(function (s, a) { return s + (a.distanceMeters || 0); }, 0) / 1000;
        const runs = arr;
        const paces = arr.map(function (a) { return a.avgPaceSecPerKm; }).filter(function (p) { return p && p > 0; });
        const hrs = arr.map(function (a) { return a.avgHr; }).filter(function (h) { return h; });
...
/** 步频分析（平均 + 步频-配速关系 + 分布）*/
export async function cadenceAnalysis(store) {
    const data = await store.read();
...
/** 爬升分析（爬升-配速影响 + 爬升分布）*/
export async function elevationAnalysis(store) {
    const data = await store.read();
...
/** 卡路里效率（每公里卡路里 + 月趋势）*/
export async function calorieEfficiency(store) {
    const data = await store.read();
...
export async function trainingConsistency(store) {
    const data = await store.read();
    ...
    // 连续训练日
    let longestStreak = 0, cur = 0, prevDate = null;
    acts.forEach(function (a) {
...
/** 大看板聚合（包含所有分析）*/
export async function fullDashboardStats(store) {
    const [paceByDistance, paceDistribution, hrPaceRelationship, trainingLoad, distanceDistribution, weekOverWeek, cadence, elevation, calories, consistency,] = await Promise.all([
...
// 别名（避免循环依赖）
async function paceByDistanceFn(s) { return paceByDistance(s); }
async function paceDistributionFn(s) { return paceDistribution(s); }
async function hrPaceRelationshipFn(s) { return hrPaceRelationship(s); }
async function trainingLoadFn(s) { return trainingLoad(s); }
async function distanceDistributionFn(s) { return distanceDistribution(s); }
async function weekOverWeekFn(s) { return weekOverWeek(s); }
async function cadenceAnalysisFn(s) { return cadenceAnalysis(s); }
async function elevationAnalysisFn(s) { return elevationAnalysis(s); }
async function calorieEfficiencyFn(s) { return calorieEfficiency(s); }
async function trainingConsistencyFn(s) { return trainingConsistency(s); }
export async function generateInsights(store) {
    const insights = [];
    const data = await store.read();
    ...
    if (runs.length >= 5) {
        ...
        const paces = recent5.map(function (a) { return a.avgPaceSecPerKm; }).filter(function (p) { return p !== undefined; });
        if (paces.length >= 3) {
...
/** 计算报告窗口 */
function reportWindow(period, from, to) {
    ...
    if (period === 'custom' && from) {
        ...
        if (period === 'week')
            ...
            start.setDate(end.getDate() - 6); // default week
    }
    return { from: start, to: end };
}
/** 生成报告聚合数据 */
export async function reportStats(store, opts = {}) {
    ...
    const totalCalories = acts.reduce(function (s, a) { return s + (a.calories || 0); }, 0);
    const paces = runs.map(function (a) { return a.avgPaceSecPerKm; }).filter(function (p) { return p !== undefined && p > 0; });
    ...
    const hrs = runs.map(function (a) { return a.avgHr; }).filter(function (h) { return h !== undefined; });
    ...
    const bestPaces = segDefs.map(function (seg) {
        ...
        if (!matches.length)
            return { label: seg.label, bestPace: '—', date: null };
    ...
    while (cur <= to) {
        ...
        if (dd) {
            ...
            if (dd.restingHeartRate) {
                rhrSum += dd.restingHeartRate;
                rhrN++;
            }
            // 压力只统计 stressAvg > 0 的（过滤 -1 / UNKNOWN / 0）
            if (dd.stressAvg && dd.stressAvg > 0) {
                stressSum += dd.stressAvg;
                stressN++;
            }
            if (dd.bodyBattery != null) {
                bbSum += dd.bodyBattery;
                bbN++;
            }
        }
...
/** 生成训练计划数据（供 AI 个性化建议）*/
export async function trainingPlanData(store) {
    const data = await store.read();
    const acts = Object.values(data.activities).sort(function (a, b) {
        return (a.startTime || '').localeCompare(b.startTime || '');
    });
    const runs = acts.filter(function (a) { return a.sport === 'running'; });
    // 基线
    const allPaces = runs.map(function (a) { return a.avgPaceSecPerKm; }).filter(function (p) { return p && p > 0; });
    ...
    const longest = runs.length ? Math.max.apply(null, runs.map(function (a) { return a.distanceMeters || 0; })) / 1000 : 0;
    const hrs = runs.map(function (a) { return a.avgHr; }).filter(function (h) { return h; });
    const avgHr = hrs.length ? Math.round(hrs.reduce(function (s, h) { return s + h; }, 0) / hrs.length) : null;
    const cads = runs.map(function (a) { return a.avgCadence; }).filter(function (c) { return c; });
    ...
    if (runs.length >= 3) {
        if (bestPace && bestPace < 300)
            assessment = '速度型跑者（5km 可进 25 分），有氧耐力需加强';
        else if (bestPace && bestPace < 330)
            assessment = '中高级跑者（5km 25-30 分），基础良好，可专注专项训练';
        else if (bestPace && bestPace < 360)
            assessment = '中级跑者（配速 5:30-6:00/km），有氧基础不错，可逐步增加强度';
        else
            assessment = '入门级跑者，重点是建立有氧基础和训练习惯';
    }
    const fingerprint = [
...
export async function hrZoneBreakdown(store) {
    const data = await store.read();
    const activities = Object.values(data.activities);
