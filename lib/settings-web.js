    ...
    return async (req, res) => {
        try {
            ...
            if (body.action === 'save') {
                ...
                // 检测账号变化：isCn（区域）变了 → 清掉旧 token（强制重新连接）。
                // email 不再持久化，故不再按 email 判断账号变更。
                const oldValue = deps.getValue();
                const newValue = body.value ?? {};
                const accountChanged = !!(oldValue && oldValue.isCn !== newValue.isCn);
                await deps.save(newValue, body.expectedRevision);
                if (accountChanged) {
                    try {
                        await deps.clearGarminTokens();
                        logger.info('settings-web', `检测到账号区域变更（isCn），已清掉旧 token`);
                    }
                    catch (e) {
