...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GarminSettingsSection() {
      ...
      return jsxs('div', {
        ...
        children: [
          ...
          jsx('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [
            jsx('span', { style: { display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%', background: draft.status === 'connected' ? '#46a758' : '#e5484d' } }),
            jsx('span', { style: { fontSize: '13px', fontWeight: '600', color: 'var(--dsw-alias-label-primary)' }, children: draft.status === 'connected' ? '已连接' : '未连接' }),
          ] }),
          jsx('div', { className: 'garmin-settings-row', children: [
            jsx('span', { className: 'label', children: '当前用户' }),
            jsx('span', { className: 'value', children: draft.displayName || '—' }),
          ] }),
          jsx('div', { className: 'garmin-settings-row', children: [
            jsx('span', { className: 'label', children: '上次同步' }),
            jsx('span', { className: 'value', children: draft.lastSyncAt ? fmtSyncTime(draft.lastSyncAt) : '—' }),
          ] }),
          jsx('div', { className: 'garmin-settings-divider' }),
          ...
          jsx('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--dsw-alias-label-tertiary)' }, children: '🔒 邮箱/密码仅用于本次登录，不会保存到本地；token 过期后需重新输入。' }),
          jsx('label', { htmlFor: 'garmin-days', children: '同步最近天数（1-' + SYNC_DAYS_BACK_MAX + '）' }),
          jsx('input', {
          ...
          jsx('div', { className: 'garmin-settings-section', children: [
            jsx('div', { className: 'garmin-settings-actions', children: [
              jsx('button', {
                onClick: doConnect,
                disabled: busy,
                className: 'garmin-btn garmin-btn-primary',
                children: busy ? '处理中…' : (mfaRequired ? '提交验证码' : (draft.status === 'connected' ? '重新连接' : '连接 Garmin')),
              }),
              jsx('button', {
          ...
          // 同步区块
          jsx('div', { className: 'garmin-settings-section', children: [
            jsx('div', { className: 'garmin-settings-section-title', children: '同步数据' }),
            jsx('div', { className: 'garmin-settings-actions', children: [
              jsx('button', {
            ...
            jsx('div', { className: 'garmin-settings-section-title', style: { marginTop: '6px' }, children: '全量同步（只活动）' }),
            jsx('div', { className: 'garmin-sync-all-line', children: [
              jsx('span', { className: 'label', style: { minWidth: 'auto' }, children: '起始日期' }),
              jsx('input', {
                type: 'date',
                value: fullSyncFrom,
              ...
              jsx('button', {
                ...
                className: 'garmin-btn',
                children: fullSyncBusy ? '全量同步中…' : '开始全量同步',
              }),
            ] }),
          ...
          jsx('div', { style: { marginTop: '4px' }, children: [
            error ? jsx('p', { style: { color: '#e5484d', fontSize: '12px', margin: '2px 0' }, children: error }) : null,
            message ? jsx('p', { style: { color: '#46a758', fontSize: '12px', margin: '2px 0' }, children: message }) : null,
          ] }),
        ],
      });
    }
    ...
    var DASH_CSS =
      ...
      '.garmin-coach-settings-card{background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);border-radius:14px;padding:18px;display:flex;flex-direction:column;gap:14px}' +
      '.garmin-coach-settings-card h3{margin:0;font-size:16px;font-weight:700;color:var(--dsw-alias-label-primary)}' +
      '.garmin-settings-row{display:flex;align-items:center;gap:8px;font-size:13px}' +
      '.garmin-settings-row .label{color:var(--dsw-alias-label-tertiary);min-width:64px}' +
      '.garmin-settings-row .value{color:var(--dsw-alias-label-primary);font-weight:500}' +
      '.garmin-settings-divider{height:1px;background:var(--dsw-alias-border-l1);margin:2px 0}' +
      '.garmin-settings-section{display:flex;flex-direction:column;gap:8px}' +
      '.garmin-settings-section-title{font-size:12px;color:var(--dsw-alias-label-tertiary);font-weight:600;text-transform:uppercase;letter-spacing:.5px}' +
      '.garmin-settings-actions{display:flex;gap:10px;flex-wrap:wrap}' +
      '.garmin-btn{border-radius:8px;padding:8px 16px;font-size:13px;cursor:pointer;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);transition:filter .15s}' +
      '.garmin-btn:hover{filter:brightness(1.08)}' +
      '.garmin-btn:disabled{opacity:.5;cursor:not-allowed}' +
      '.garmin-btn-primary{background:var(--dsw-alias-accent-color,#4a7dff);color:#fff;border-color:transparent}' +
      '.garmin-sync-status{font-size:12px;color:var(--dsw-alias-label-secondary);min-height:16px}' +
      '.garmin-sync-status.error{color:#e5484d}' +
      '.garmin-sync-status.success{color:#46a758}' +
      '.garmin-sync-all-line{display:flex;align-items:center;gap:8px;flex-wrap:wrap}' +
      '.garmin-sync-all-line input[type=date]{padding:6px 8px;font-size:12px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary)}';
    ...
    function GarminDashboard(props) {
      ...
      var HEALTH_DETAIL_GROUPS = [
        ...
        { label: 'HRV', items: [
          { label: '状态', dep: 'hrvStatus', get: function (d) { return d.hrvStatus || '—' } },
          { label: '周均', dep: 'hrvWeeklyAvg', get: function (d) { return d.hrvWeeklyAvg != null ? d.hrvWeeklyAvg + ' ms' : '—' } },
        ] },
        { label: '训练准备度', items: [
          { label: '准备度分', dep: 'readinessScore', get: function (d) { return d.readinessScore != null ? d.readinessScore + ' /100' : '—' } },
        ] },
      ];

      // 健康表默认显示列 + 全部可选列
      var HEALTH_DEFAULT_COLS = ['steps', 'restingHeartRate', 'minHeartRate', 'maxHeartRate'];
      var HEALTH_ALL_COLS = [
        { key: 'steps', label: '步数' },
        ...
        { key: 'hrvStatus', label: 'HRV 状态' },
        { key: 'hrvWeeklyAvg', label: 'HRV 周均' },
        { key: 'readinessScore', label: '训练准备度' },
      ];
      ...
      var renderHealthCell = function (key, dd) {
        switch (key) {
          ...
          case 'hrvStatus': return dd.hrvStatus || '—';
          case 'hrvWeeklyAvg': return dd.hrvWeeklyAvg != null ? dd.hrvWeeklyAvg + ' ms' : '—';
          case 'readinessScore': return dd.readinessScore != null ? String(dd.readinessScore) : '—';
          default: return '—';
        }
      };
