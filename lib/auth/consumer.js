...
import { OAUTH_CONSUMER_CACHE_TTL_SEC, OAUTH_CONSUMER_FALLBACK_KEY, OAUTH_CONSUMER_FALLBACK_SECRET, OAUTH_CONSUMER_URL, } from './constants.js';
...
async function saveCache(c) {
    try {
        ...
        await writeFile(CACHE_PATH, JSON.stringify({ ...c, fetchedAt: Date.now() }), 'utf8');
    }
    catch {
        /* 缓存失败无所谓，下次再拉 */
    }
}
export async function loadConsumerCreds() {
...
export async function invalidateConsumerCache() {
    if (existsSync(CACHE_PATH)) {
