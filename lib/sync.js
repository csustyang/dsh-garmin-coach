 ...
 */
export function isEmptyDailyPayload(raw) {
    if (!raw || typeof raw !== 'object')
        return true;
    const s = raw;
    // 各个字段"有意义值"的判定函数
    const num = (v) => typeof v === 'number' && v > 0;
    const truthy = (v) => v != null && v !== '' && v !== false;
    // 心率真采样：min ≠ max（真采样的心率一定有波动，min=75 max=75 是 Garmin 默认值/未采样）
    const hasRealHeartRate = typeof s.minHeartRate === 'number' &&
    ...
    const allDayInactive = (s.activeSeconds === 0 || s.activeSeconds == null) &&
 ...
 */
function mergeDailyRaws(dailyRaw, hrvRaw, readinessRaw, trainingRaw) {
    const merged = {};
...
export function toDailyRecord(date, raw) {
    ...
    return {
        ...
        sleepSeconds: sleepDto?.sleepTimeSeconds ?? s.sleepingSeconds,
        ...
        for (let i = 0; i < effectiveDays; i++) {
            ...
            try {
                ...
                const extrasPromise = Promise.all([
                    ...
                    logger.warn('sync', `${dateStr} training 补全失败: ${e.message}`);
                });
...
export async function syncAllActivities(store, queries, opts) {
    ...
    if (!opts.resume) {
        try {
            ...
            logger.warn('syncAll', '备份失败（继续同步）: ' + e.message);
        }
    }
    const cleanupBackup = async () => {
        if (backupPath) {
            try {
                ...
                logger.error('syncAll', '清理备份失败: ' + e.message);
            }
        }
    };
    const restoreBackup = async () => {
        if (backupPath) {
            try {
                ...
                logger.error('syncAll', '恢复备份失败: ' + be.message);
            }
        }
    };
    try {
