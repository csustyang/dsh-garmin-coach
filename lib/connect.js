 ...
 */
export function makeConnectHandler(getClient, updateStatus, store, 
...
export function installConnectRoute(ctx, handler) {
    const anyCtx = ctx;
    if (!anyCtx.inject) {
    ...
    anyCtx.inject(['webServer'], (webCtx) => {
        ...
        webCtx.effect?.(() => dispose, 'dsh-garmin-coach: connect route');
