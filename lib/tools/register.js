 ...
 */
export function defineGarminTools(q) {
    const dateSpec = {
        type: 'string',
    ...
    return [
        ...
        defineGarminTool({
            ...
            execute: async (args) => q.activities({
                from: args.from,
                to: args.to,
                limit: args.limit,
            }),
        ...
        defineGarminTool({
            ...
            parameters: { date: dateSpec },
            execute: async (args) => q.summary(args.date),
        }),
        defineGarminTool({
            name: 'garmin_weekly',
            description: '返回 7 天窗口（含每日 daily+sleep + 当周活动）。结束日期默认今天。',
            parameters: { endDate: dateSpec },
            execute: async (args) => q.weekly(args.endDate),
        }),
    ];
}
