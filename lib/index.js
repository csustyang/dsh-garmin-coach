...
import { GARMIN_SETTINGS_NS, installGarminSettingsRoute, makeGarminSettingsHandler, } from './settings-web.js';
 ...
 */
function safeSync(scope, fallback, fn) {
    try {
        return fn();
    }
    catch (e) {
 ...
 */
async function safeAsync(scope, fallback, fn) {
    try {
        return await fn();
    }
    catch (e) {
...
// ────────────────────────────────────────────────────────────────────────
function makeCordisTokenStore(creds) {
    return {
        async loadTokens() {
            const r = await safeAsync('token.loadTokens', undefined, () => creds.get('garmin_tokens'));
            return r?.value ?? null;
        },
        async saveTokens(t) {
        ...
        async loadMfaState() {
            const r = await safeAsync('token.loadMfaState', undefined, () => creds.get('garmin_mfa_state'));
            return r?.value ?? null;
        },
...
// ────────────────────────────────────────────────────────────────────────
function tryRegisterTools(ctx, tools) {
    if (!ctx.tools) {
...
function tryRegisterSettings(ctx, store) {
    ...
    safeSync('apply.settings', undefined, () => {
        const settingsScope = ctx.settings.register(SETTINGS_NAMESPACE, GarminSettingsSchema, {
...
function tryRegisterCommands(ctx, queries) {
    if (!ctx.commands) {
 ...
 */
async function syncOnConnect(store, queries, getSettings) {
    if (!store || !queries)
        return undefined;
    try {
...
export function apply(rawCtx) {
    try {
        ...
        const store = safeSync('apply.store', FileTokenStore.default(), () => FileTokenStore.default());
        // 从 settings 读 isCn（区域），决定用 garmin.cn 还是 garmin.com
        const initialSettings = (() => {
            try {
                const s = rawCtx.settings;
        ...
        const garminStore = safeSync('apply.garminStore', null, () => new GarminStoreFile());
        // 统计查询工具（基于本地落库数据，不实时调 Garmin API）
        const statsTools = safeSync('apply.statsTools', [], () => (garminStore ? defineStatsTools({ store: garminStore }) : []));
        // 各 seam 注册（每个内部已包）
        ...
        if (garminStore) {
            ;
            globalThis.__garmin_store = garminStore;
        ...
        safeSync('apply.settingsRoute', undefined, () => {
            ...
            /** 全量同步实时进度（内存态，供前端轮询）*/
            const fullSyncState = { status: 'idle', processed: 0, total: 0, activitiesAdded: 0, activitiesTotal: 0 };
            const handler = makeGarminSettingsHandler({
                ...
                save: async (value, expectedRevision) => {
                    try {
                        ...
                        const { email, password, ...safe } = (value ?? {});
                ...
                connect: async (email, password, mfaCode) => {
                    try {
                        ...
                        const markConnected = async (displayName) => {
                            try {
                                const s = rawCtx.settings;
                    ...
                    try {
                        ...
                        void (async () => {
                            try {
                                ...
                                if (result.error)
                                    fullSyncState.error = result.error;
                ...
                dashboard: async () => {
                    if (!garminStore)
                        return { error: true, message: '数据存储未就绪' };
