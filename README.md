...
或在 DSH web UI 的 **设置 → 插件 → 添加插件** 中搜索 `dsh-garmin-coach`。
...
7. 点击**立即同步** → 拉取最近 14 天数据

> ⚠️ **首次同步后**，数据存到 `~/data/garmin.json`（DSH 用户的 Home 目录）。
...
- **增量**：每次只拉上次同步后的新数据（lastSyncAt 之后）
